...
class FormatConfigManager:
    def __init__(self):
        ...
        self._user_format_configs: dict[str, FormatConfig] = {}
        ...
        if FORMAT_CONFIG_FILE.exists():
            ...
            for format_type, format_config in data.items():
                self._format_configs[format_type] = FormatConfig(
                    format_config.get("log_format", ""), format_config.get("regex", [])
                )
        ...
        if USER_FORMAT_CONFIG_FILE.exists():
            ...
            for format_type, format_config in user_data.items():
                self._user_format_configs[format_type] = FormatConfig(
                    format_config.get("log_format", ""), format_config.get("regex", [])
                )

    def get_all_format_types(self) -> list[str]:
        return list(self._format_configs.keys()) + list(self._user_format_configs.keys())

    def get_format_config(self, format_type: str) -> FormatConfig | None:
        return self._format_configs.get(format_type) or self._user_format_configs.get(format_type)

    def save_custom_format(self, format_type: str, log_format: str, regex: list[str]):
        # 保存为 FormatConfig 对象
        self._user_format_configs[format_type] = FormatConfig(log_format, regex)

        with open(USER_FORMAT_CONFIG_FILE, "w", encoding="utf-8") as f:
            yaml.safe_dump(
                {k: asdict(v) for k, v in self._user_format_configs.items()},
                f,
                allow_unicode=True,
                indent=2,
                width=float("inf"),
    ...
    def is_format_type_exists(self, format_type: str) -> bool:
        return format_type in self._format_configs or format_type in self._user_format_configs
