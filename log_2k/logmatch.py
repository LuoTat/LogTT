...
# ]
benchmark_settings = {
    "HDFS": {"log_file": "HDFS/HDFS.log", "log_format": "<Date> <Time> <Pid> <Level> <Component>: <Content>", "regex": [r"blk_-?\d+", r"(\d+\.){3}\d+(:\d+)?"], "st": 0.5, "depth": 4},
    "Hadoop": {"log_file": "Hadoop/Hadoop.log", "log_format": "<Date> <Time> <Level> \[<Process>\] <Component>: <Content>", "regex": [r"(\d+\.){3}\d+"], "st": 0.5, "depth": 4},
    "Spark": {"log_file": "Spark/Spark.log", "log_format": "<Date> <Time> <Level> <Component>: <Content>", "regex": [r"(\d+\.){3}\d+", r"\b[KGTM]?B\b", r"([\w-]+\.){2,}[\w-]+"], "st": 0.5, "depth": 4},
    "Zookeeper": {"log_file": "Zookeeper/Zookeeper.log", "log_format": "<Date> <Time> - <Level>  \[<Node>:<Component>@<Id>\] - <Content>", "regex": [r"(/|)(\d+\.){3}\d+(:\d+)?"], "st": 0.5, "depth": 4},
    "BGL": {"log_file": "BGL/BGL.log", "log_format": "<Label> <Timestamp> <Date> <Node> <Time> <NodeRepeat> <Type> <Component> <Level> <Content>", "regex": [r"core\.\d+", r"\d+:[a-fA-F0-9]{8,}", r"[a-fA-F0-9]{8,}"], "st": 0.6, "depth": 5},
    "HPC": {"log_file": "HPC/HPC.log", "log_format": "<LogId> <Node> <Component> <State> <Time> <Flag> <Content>", "regex": [r"=\d+"], "st": 0.5, "depth": 4},
    "Thunderbird": {"log_file": "Thunderbird/Thunderbird.log", "log_format": "<Label> <Timestamp> <Date> <User> <Month> <Day> <Time> <Location> <Component>(\[<PID>\])?: <Content>", "regex": [r"(\d+\.){3}\d+"], "st": 0.5, "depth": 4},
    "Windows": {"log_file": "Windows/Windows.log", "log_format": "<Date> <Time>, <Level>                  <Component>    <Content>", "regex": [r"0x.*?\s"], "st": 0.7, "depth": 5},
    "Linux": {"log_file": "Linux/Linux.log", "log_format": "<Month> <Date> <Time> <Level> <Component>(\[<PID>\])?: <Content>", "regex": [r"(\d+\.){3}\d+", r"\d{2}:\d{2}:\d{2}"], "st": 0.39, "depth": 6},
    "Android": {"log_file": "Android/Android.log", "log_format": "<Date> <Time>  <Pid>  <Tid> <Level> <Component>: <Content>", "regex": [r"(/[\w-]+)+", r"([\w-]+\.){2,}[\w-]+", r"\b(\-?\+?\d+)\b|\b0[Xx][a-fA-F\d]+\b|\b[a-fA-F\d]{4,}\b"], "st": 0.2, "depth": 6},
    "HealthApp": {"log_file": "HealthApp/HealthApp.log", "log_format": "<Time>\|<Component>\|<Pid>\|<Content>", "regex": [r"\d+##\d+##\d+##\d+##\d+##\d+", r"=\d+"], "st": 0.2, "depth": 4},
    "Apache": {"log_file": "Apache/Apache.log", "log_format": "\[<Time>\] \[<Level>\] <Content>", "regex": [r"(\d+\.){3}\d+"], "st": 0.5, "depth": 4},
    "Proxifier": {"log_file": "Proxifier/Proxifier.log", "log_format": "\[<Time>\] <Program> - <Content>", "regex": [r"<\d+\ssec", r"([\w-]+\.)+[\w-]+(:\d+)?", r"\d{2}:\d{2}(:\d{2})*", r"[KGTM]B"], "st": 0.6, "depth": 3},
    "OpenSSH": {"log_file": "OpenSSH/OpenSSH.log", "log_format": "<Date> <Day> <Time> <Component> sshd\[<Pid>\]: <Content>", "regex": [r"(\d+\.){3}\d+", r"([\w-]+\.){2,}[\w-]+"], "st": 0.6, "depth": 5},
    "OpenStack": {"log_file": "OpenStack/OpenStack.log", "log_format": "<Logrecord> <Date> <Time> <Pid> <Level> <Component> \[<ADDR>\] <Content>", "regex": [r"\[instance:\s*(.*?)\]", r"((\d+\.){3}\d+,?)+", r"/.+?\s", r"\d+"], "st": 0.2, "depth": 4},
    "Mac": {"log_file": "Mac/Mac.log", "log_format": "<Month>  <Date> <Time> <User> <Component>\[<PID>\]( \(<Address>\))?: <Content>", "regex": [r"([\w-]+\.){2,}[\w-]+"], "st": 0.7, "depth": 6},
}
